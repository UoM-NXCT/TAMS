...
class NikonScan(AbstractScan):
    ...
    def __init__(self: NikonScan, path: Path) -> None:
        """Initialize the scan.
    ...
    def get_metadata(self: NikonScan) -> dict[str, Any] | None:
        ...
        try:
            ...
            msg = "Could not find .ctprofile.xml file"
        ...
        try:
            ...
            msg = "Could not find .xtekct file"
            raise FileNotFoundError(msg) from exc
        ...
        try:
            ...
            voltage_kv = int(settings.find("kV").text)
            current_ua = int(settings.find("uA").text)
        ...
        xtekct_data = ConfigParser()
        xtekct_data.read(xtekct_file)
        scan_name = xtekct_data["XTekCT"]["Name"]
    ...
    def is_reconstruction(path: Path) -> bool:
        """Check if a directory is a reconstruction directory.
    ...
    def get_reconstructions(self: NikonScan) -> tuple[Path, ...]:
    ...
    def get_raw_data(self: NikonScan) -> tuple[Path, ...]:
